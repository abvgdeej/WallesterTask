...
/**
 * Deserializer of a {@link com.wallester.backend.domain.dto.CustomerDto#birthDate} to dd.MM.yyyy format with right
 * date regex.
 */
public class DateDeSerializer extends StdDeserializer<Date> {
    ...
    public final Pattern pattern = Pattern.compile(DATE_REGEX);
    ...
    public Date deserialize(JsonParser p, DeserializationContext ctxt)
        ...
        if (!pattern.matcher(value).find()) {
            throw new ApiException("Date validation error", 400);
        }
        try {
